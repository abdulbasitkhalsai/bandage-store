...
// // Define NextAuth configuration with explicit AuthOptions type
// const authOptions: AuthOptions = {
//   providers: [
//     GoogleProvider({
//       clientId: process.env.GOOGLE_CLIENT_ID!,
...
//     async session({ session, token }: { session: Session; token: JWT }) {
//       if (session.user) {
//         session.user.id = token.sub || ""; // Set the id to the token's sub
//         session.user.name = token.name || "Guest"; // Add a fallback for the name
//       }
//       return session;
//     },
//   },
//   session: {
//     strategy: "jwt", // Use string 'jwt' for session strategy
//   },
//   secret: process.env.NEXTAUTH_SECRET,
...
// const handler = NextAuth(authOptions); // Initialize NextAuth with the options
// export { handler as GET, handler as POST }; // Export the handler


// code of [...next-auth]/route.ts
...
export const authOptions = {
  ...
  callbacks: {
    async signIn({ user }: { user: User }) { // Type the user parameter
      try {
        ...
        return false; // This prevents login
      }
    },
  },
};
