...
const SingleProd = ({params} : {params : {id :number}}) => {  
  const rawData = FeaturedProd
  const prod = rawData.find((item) => item.id === Number(params.id))
  return (
    <div className='bg-[#FAFAFA]'>
      ...
      <div className='flex flex-col lg:flex-row gap-[30px]'>
        <div className='flex flex-col gap-5'>
          <Image className='md:w-[506px] w-fit h-[450px]' src={prod.image} alt={prod.altImage} width={506} height={450}></Image>
          <div className='flex gap-5'>
            <Image className='w-fit md:w-100 h-[75px]' src={prod.image} alt={prod.altImage} width={100} height={75}></Image>
            <Image className='w-fit md:w-100 h-[75px]' src={prod.image} alt={prod.altImage} width={100} height={75}></Image>
          </div>
        </div>
        <div className='flex flex-col text-left gap-2 p-3 max-w-[510px]'>
          <div className='font-normal text-xl tracking-[0.2px]'>{prod.title}</div>
          ...
          <div className='opacity-50'><del>${prod.price}</del></div>
          <div>${prod.discPrice}</div>
          </div>
          <div className='flex gap-2'>
            <span className='text-[#737373]'>Availiblity :</span>
            <span className='text-[#23A6F0]'>In Stock</span>
          </div>
          <div className='mt-5 text-sm font-normal text-wrap text-[#858585]'>{prod.description}</div>
    ...
    {!prod && <div className='h-24 w-full text-center flex items-center justify-center font-bold text-3xl'>Product Not Found Back to the <button className='rounded-md bg-black m-2'><Link className='px-4 py-6 text-white' href={"/"}>Home Page</Link></button></div>}
