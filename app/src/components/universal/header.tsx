...
const Header = ({ children }: { children?: ReactNode }) => {
  ...
  const navigation = useMemo(
    () =>
      navItems.map((item, index) => (
        <li
          key={index}
          className="relative lg:hover:after:absolute lg:after:bg-white lg:after:w-0 lg:hover:after:w-full lg:hover:after:h-[2px] lg:after:block lg:after:-bottom-4 lg:after:transition-all lg:after:duration-300"
        >
          <Link href={item.link} className="hover:text-black">
            {item.title}
          </Link>
  ...
  return (
    <header className="font-[sans-serif] min-h-[40px] tracking-wide z-50 shadow-md sticky top-0 w-full bg-white">
      <div>{children}</div>
      <div className="wrapper">
        <div className="flex flex-wrap items-center py-3 px-4 gap-y-4 gap-x-4 justify-between w-full overflow-hidden">
          {/* Brand Logo */}
          ...
          <nav className="flex flex-1 w-fit lg:w-auto justify-end lg:justify-between items-center sm:ml-4">
            ...
            <div className="flex items-center max-sm:ml-auto ml-6 gap-4">
              ...
              {status === "loading" ? (
                <div className="w-36 h-6 bg-gray-200 animate-pulse rounded" />
              ) : session ? (
                <div className="flex items-center gap-4">
                  {/* User Avatar */}
                  <Image
